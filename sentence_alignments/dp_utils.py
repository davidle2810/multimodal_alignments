...
def read_alignments(fin):
    ...
    return alignments
'''
def print_alignments(alignments, text, scores=None, src_lines=None, tgt_lines=None, ofile=sys.stdout):
    list_vis = []
    list_los = []
    alignment_list = []
    if scores is not None:
        for i, tmp in enumerate(zip(text, scores)):
            x,s=tmp
            x = x.replace('<==>', '\t')
            vi = x.split('\t')[0].strip()
            lo = x.split('\t')[1].strip()
            ...
            for line in vi.split(' '):
                if line.strip() in list_vis:
                    mark = True
            for line in lo.split(' '):
                if line.strip() in list_los:
                    mark = True
            if len(vi.split(' ')) <= len(lo.split(' ')) and len(set(vi.split()).intersection(set(lo.split(' '))))*1.0 / len(vi.split(' '))>=0.2:
                if mark is False:
                    # Use Vietnamese-specific sentence tokenization for both source and target
                    vi_sentences = vi.split(' ')  # Tokenize source sentence
                    lo_sentences = lo.split(' ')  # Tokenize target sentence
                    list_vis.append(vi)
                    list_los.append(lo)
                    #print('%.4f\t%s' % (s_c, x), file=ofile)
                    alignment_list.append(alignments[i])

    for (x, y), s in zip(alignment_list, scores):
        if s is None:
    ...
    with open('./data/output.txt', 'w') as f:
        for (x, y), s in zip(alignments, scores):
            if s is None:
                ...
                f.write(f'{x}:{y}:{s:.6f}\n')
            if src_lines is not None and tgt_lines is not None:
                src_text = ' '.join([src_lines[i].strip().replace('\n', ' ') for i in x])
                tgt_text = ' '.join([tgt_lines[i].strip().replace('\n', ' ') for i in y])
                f.write(f'{" "*40}SRC: {src_text}\n')
                f.write(f'{" "*40}TGT: {tgt_text}\n')
...
def vecalign(vecs0,
    ...
    '''
    vis_raw_lines = io.open('data/test1.tr', encoding='utf-8').read().split('\n')
    los_raw_lines = io.open('data/test1.vi', encoding='utf-8').read().split('\n')

    final_alignment_texts = []
    for (id_srcs, id_tgts) in stack[0]['final_alignments']:
        result_src = '[]'
        result_tgt = '[]'
        if id_srcs != []:
            result_src = " ".join([vis_raw_lines[int(idx)] for idx in id_srcs])
        if id_tgts != []:
            result_tgt = " ".join([los_raw_lines[int(idx)] for idx in id_tgts])
        final_alignment_texts.append(result_src.replace('\n', "") + " <==> " + result_tgt.replace('\n', ""))
    stack[0]['final_alignments_text'] = final_alignment_texts'''
